    ...
    <div className="min-h-screen flex flex-col bg-gray-100" style={{ fontSize: "13px" }}>
      ...
      <div className="text-white text-[13px] py-0 px-0 flex items-stretch shadow-lg z-10 min-h-[36px]" style={{ backgroundColor: BRAND }}>
        ...
        <span className="px-4 py-2 border-r border-white/20 shrink-0 flex items-center">User Role :
          <br />{user?.userRoleLabel || "College"}</span>
        <span className="px-4 py-2 border-r border-white/20 shrink-0 flex items-center">{user?.aisheCode || "C-10034"}</span>
        <span className="px-4 py-2 border-r border-white/20 flex items-center font-medium tracking-wide flex-1 min-w-0 leading-tight">{user?.instituteName || "PEOPLETREE EDUCATION SOCIETY'S BBA/BCA AND BCOM COLLEGE, AP NEHARU NAGAR, BELAGAVI"}</span>
        <span className="px-4 py-2 border-r border-white/20 shrink-0 flex items-center">State :
          <br />{user?.state || "Karnataka"}</span>
        <div className="relative shrink-0" ref={dropdownRef}>
          ...
          >
            ...
            <span className="font-semibold tracking-wide text-[12px]">{user?.username || "ptesbba"}</span>
          </button>
          {userDropdownOpen && (
