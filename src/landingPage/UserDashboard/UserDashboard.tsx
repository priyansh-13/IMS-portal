...
const tiles = [
  ...
  },
  {
    ...
    icon: GraduationCap,
    // href will be determined dynamically based on role
    href: "/aishe",
    completion: 50,
    status: "In Progress" as const,
  },
...
const Dashboard = () => {
  ...
  // Determine AISHE href based on user role
  const getAisheHref = () => {
    if (!user) return "/aishe";
    switch (user.role) {
      case "university":
        return "/aishe";
      case "college":
        return "/aishe-college";
      case "standalone":
        return "/aishe-standalone";
      default:
        return "/aishe";
    }
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-navy-light text-primary-foreground py-2 px-4 text-xs">
        <div className="container mx-auto flex gap-8 flex-wrap">
          <span>User Role: {user?.userRoleLabel || "Unknown"}</span>
          <span>AISHE Code: {user?.aisheCode || "N/A"}</span>
          <span>{user?.instituteName || "Unknown Institute"}</span>
      ...
      <main className="container mx-auto py-10 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {tiles.map((tile) => {
            const href = tile.title === "AISHE" ? getAisheHref() : tile.href;
            return (
              <Link
                key={tile.title}
                to={href}
